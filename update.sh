...
# Your own files (your AGENTS.md, your vault, your configs) live outside
...
main() {
  for repo in fullstack-agent ai-memory-vault backtalk barehands ai-visualizer; do
    ...
    if [ ! -d "$HOME_DIR/$repo/.git" ]; then
      ...
      if [ "$repo" = "fullstack-agent" ]; then
        git -C "$HOME_DIR/$repo" remote add origin "https://github.com/davisoliveira1520-cmd/fullstack-agent-opencode"
      else
